...
fn status_commands_report_client_and_server_versions() {
    ...
    assert!(
        full_stdout.contains("  protocol: 27"),
        "stdout: {full_stdout}"
    );
    ...
    assert!(
        server_stdout.contains("protocol: 27"),
        "stdout: {server_stdout}"
    );
    ...
    assert!(
        client_stdout.contains("protocol: 27"),
        "stdout: {client_stdout}"
    );
    assert!(
    ...
    assert_eq!(full_json["client"]["protocol"], 27);
    ...
    assert_eq!(server_json["protocol"], 27);
    ...
    assert_eq!(client_json["protocol"], 27);
