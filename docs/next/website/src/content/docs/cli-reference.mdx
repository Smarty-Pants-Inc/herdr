...
Per-pane SSH execution targets are Unix-only. This is separate from whole-server
`herdr --remote`, which remains available on Windows clients connecting to
supported Linux and macOS hosts.

Without `--target`, a split inherits the source pane's execution target. Use
`--target local` to force local execution or `--target ssh:HOST` to run the new
terminal on a trusted SSH host while keeping it in the current Herdr UI. A
same-target split follows the source cwd. A cross-target local split uses the
configured `terminal.new_cwd`; a cross-target SSH split starts in the remote
user's home. An explicit `--cwd` overrides either default. Prepare a matching
remote Herdr first by running `herdr --remote HOST`; per-pane launches never
install or replace remote software. See [Persistence and remote access](/docs/persistence-remote/).

Per-pane launches use non-interactive OpenSSH with bounded connection and request-delivery timeouts, so authentication must already be available through keys or `ssh-agent`.
...
`agent start` activates an existing available shell pane: the pane's interactive shell must own the foreground, with no foreground command, editor, or agent running. `agent start` currently supports local panes only; targeting an SSH-backed pane returns `agent_start_unsupported_execution_target`. Topology must be created separately. Names are unique among live agents and must match `[a-z][a-z0-9_-]{0,31}`. The kind selects Herdr's canonical interactive executable, while arguments after `--` are passed to that executable. Supported kinds are `pi`, `claude`, `codex`, `gemini`, `cursor`, `devin`, `agy`, `cline`, `omp`, `mastracode`, `opencode`, `copilot`, `kimi`, `kiro`, `droid`, `amp`, `grok`, `hermes`, `kilo`, `qodercli`, `qwen`, and `maki`. A name follows the current pane occupant and is cleared when that agent exits, is released, or is replaced. Temporary detection uncertainty does not clear it.
