...
```bash
herdr plugin pane open --plugin ID --entrypoint ID [--placement overlay|popup|workspace-right|split|tab|zoomed] [--width SIZE] [--height SIZE] [--workspace ID] [--target-pane PANE] [--direction right|down] [--cwd PATH] [--env KEY=VALUE] [--focus|--no-focus]
...
new tab, a zoomed pane, a session-modal `popup`, or a `workspace-right` pane.
...
does not export `HERDR_PANE_ID` and does not participate in pane or agent APIs.
Native non-terminal plugin panes are a later surface.
