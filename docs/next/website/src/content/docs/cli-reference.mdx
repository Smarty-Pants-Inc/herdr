...
When a managed workspace-right plugin invokes the command, Herdr carries its
pane identity internally so the new pane inherits the plugin's execution target
and cwd instead of an unrelated tiled pane's locality.
...
same-target split follows the source cwd. A cross-target split starts in the
destination's default home unless `--cwd` is explicit. Prepare a matching
