...
herdr pane split [<pane_id>|--pane ID|--current] --direction right|down [--workspace WORKSPACE] [--ratio FLOAT] [--cwd PATH] [--target local|ssh:HOST] [--env KEY=VALUE] [--right-click herdr|pane] [--focus] [--no-focus]
...
an explicit pane id, `--pane ID`, or `--current` uses that selected pane for
both insertion and inherited execution target/cwd. With no pane selector,
Herdr uses the UI-focused pane. The split response exposes the new pane ID as
`.result.pane.pane_id`.

`--workspace` selects that workspace's focused tiled pane as the insertion point.
When a managed workspace-right plugin invokes a workspace-selected split without
an explicit pane selector, the new pane may inherit the plugin caller's execution
target and cwd while still splitting the workspace's focused tiled pane.

Per-pane SSH and execution-provider targets require a Unix Herdr server. This
is separate from whole-server `herdr --remote`, which remains available on
Windows clients connecting to supported Linux and macOS hosts. Provider plugins
must also support the server platform.

Without `--target`, a split inherits the source pane's execution target. Use
`--target local` to force local execution, `--target ssh:HOST` for built-in SSH,
or `--target SCHEME:TARGET` for an installed execution provider. For example,
`--target runtime:dev2` resolves the enabled plugin that declares `runtime` and
passes `dev2` to it. A same-target split follows the source cwd. A cross-target
local split uses the configured `terminal.new_cwd`; a cross-target remote split
starts with the target provider's default cwd. An explicit `--cwd` overrides
that default. Built-in SSH requires a matching remote Herdr prepared through
`herdr --remote HOST`; per-pane launches never install or replace remote
software. See [Persistence and remote access](/docs/persistence-remote/) and
[Plugins](/docs/plugins/).

Built-in SSH uses non-interactive OpenSSH with bounded connection and
request-delivery timeouts, so authentication must already be available through
keys or `ssh-agent`. Execution-provider transport and authentication are owned
by the provider plugin.

`pane get` and `pane list` report each pane's `execution_target`. Remote cwd
reports come from the remote terminal and are not checked against the server's
filesystem. `pane process-info` is local-only; remote panes return
`pane_process_info_unsupported` rather than local process facts.
...
`agent start` activates an existing available shell pane: the pane's interactive shell must own the foreground, with no foreground command, editor, or agent running. Local, SSH-backed, and execution-provider-backed panes are supported. Remote panes must use a POSIX-compatible interactive shell; Herdr sends a shell-safe launch command and relies on terminal lifecycle detection instead of local process-table inspection. Topology must be created separately. Names are unique among live agents and must match `[a-z][a-z0-9_-]{0,31}`. The kind selects Herdr's canonical interactive executable, while arguments after `--` are passed to that executable. Supported kinds are `pi`, `claude`, `codex`, `gemini`, `cursor`, `devin`, `agy`, `cline`, `omp`, `mastracode`, `opencode`, `copilot`, `kimi`, `kiro`, `droid`, `amp`, `grok`, `hermes`, `kilo`, `qodercli`, `qwen`, and `maki`. A name follows the same terminal after pane moves.
...
```bash
herdr plugin pane open --plugin ID --entrypoint ID [--placement overlay|popup|workspace-right|split|tab|zoomed] [--scope shared|client-private] [--width SIZE] [--height SIZE] [--workspace ID] [--target-pane PANE] [--direction right|down] [--cwd PATH] [--env KEY=VALUE] [--focus|--no-focus]
...
with the effective execution target. It starts a manifest-declared `[[panes]]`
command as a Herdr-managed terminal surface. The manifest placement default is
`overlay`, which opens a temporary zoomed overlay over the active pane. It can
also open as a split, a new tab, a zoomed pane, a session-modal `popup`, or a
`workspace-right` pane. Scope defaults to `shared`; `--scope` overrides the
manifest. `client-private` is valid only with popup placement and uses the
calling plugin environment's `HERDR_VIEW_ID` to route the popup to its owning
connected app client.

A `workspace-right` pane occupies the active workspace's right edge outside the
tab split tree. Each workspace can own one; tab switches retain it, and
workspace switches restore that workspace's visibility and focus. `--no-focus`
starts it as a collapsed one-column strip; focusing expands it. The collapse
control itself never changes focus. `--width` sets its outer width in terminal
cells or as a percentage such as `30%`, with a default of 30%. It exports
`HERDR_WORKSPACE_ID` and `HERDR_PANE_ID` but not `HERDR_TAB_ID`. The pane is
runtime-scoped rather than snapshot-persisted and closes with its workspace, its
process, or `plugin pane close`.

For `popup`, `--width` and `--height` set the outer dimensions in terminal
cells or percentages such as `80%`; omitted dimensions default to half the
terminal size, and values smaller than the popup minimum are clamped. A shared
popup is a singleton session resource. A client-private popup belongs to one
connected app client, which can own one independent popup runtime. Both popup
scopes have no public pane ID, do not export `HERDR_PANE_ID`, and do not
participate in pane or agent APIs. Native non-terminal plugin panes are outside
plugin v1.

`--env KEY=VALUE` can be repeated on process-launching commands. It applies to the newly launched process only. Herdr-managed variables such as `HERDR_SOCKET_PATH`, `HERDR_BIN_PATH`, `HERDR_ENV`, `HERDR_WORKSPACE_ID`, `HERDR_TAB_ID`, `HERDR_PANE_ID`, `HERDR_VIEW_ID`, `HERDR_PLUGIN_ID`, `HERDR_PLUGIN_ROOT`, `HERDR_PLUGIN_CONFIG_DIR`, `HERDR_PLUGIN_STATE_DIR`, `HERDR_PLUGIN_ENTRYPOINT_ID`, and `HERDR_PLUGIN_CONTEXT_JSON` stay authoritative when they conflict with caller-provided env.
