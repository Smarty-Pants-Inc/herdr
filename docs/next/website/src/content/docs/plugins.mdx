...
`overlay`, `popup`, `workspace_right`, `split`, `tab`, or `zoomed`.
...
remain outside general pane, layout, persistence, and agent APIs. A popup is a
singleton session resource with no pane ID or pane lifecycle events; its process
does not receive `HERDR_PANE_ID`, and the underlying tiled pane remains
available through `HERDR_PLUGIN_CONTEXT_JSON`. Opening a popup returns `ui_busy`
while Settings, Copy mode, or another Herdr modal is active, and
