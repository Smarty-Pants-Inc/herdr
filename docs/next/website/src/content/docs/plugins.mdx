...
or linked on the target host. The remote Herdr resolves the target-local
executable, platform, plugin root, config directory, and state directory;
Herdr does not copy plugin packages between hosts. Startup and event hooks
remain local to the Herdr server.
