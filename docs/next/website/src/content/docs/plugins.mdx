...
plugin v1. Actions, event hooks, panes, link handlers, and execution providers
...
scheme = "runtime"
protocol = 1
pty_command = ["target/release/herdr-runtime", "connect"]
process_command = ["target/release/herdr-runtime", "exec"]
...
can run. Build commands, startup hooks, actions, event hooks, panes, link
handlers, and execution providers can also declare their own `platforms`;
item-level platforms override the top-level list. Local plugins without
top-level `platforms` link with a warning.
...
Build commands are plain argv commands too. Herdr scrubs runtime context such as
`HERDR_BIN_PATH`, socket and session variables, pane/workspace/view identity,
and all `HERDR_PLUGIN_*` variables, then injects only `HERDR_BUILD_BIN_PATH` as
the validated absolute path to the Herdr executable running the install.
Runtime plugin commands never receive `HERDR_BUILD_BIN_PATH`. Plugin authors
should document required system tools such as `cargo`, `npm`, `bun`, or `lua`;
Herdr reports build failures but does not install missing toolchains.
...
User-invoked plugin actions, panes, and link handlers follow the source pane's
execution target. Per-pane SSH execution targets are Unix-only; whole-server
`herdr --remote` remains available on Windows clients connecting to supported
Linux and macOS hosts. For an SSH target, the Herdr server uses its local
manifest for routing, while the same plugin id and entrypoint must be installed
or linked on the target host. For a plugin-declared execution provider, Herdr
resolves the action or pane to its manifest argv first and passes that argv to
the provider. Provider-backed targets therefore require commands that are
meaningful on the provider's target; Herdr does not copy plugin packages.
Startup and event hooks remain local to the Herdr server.
...
`HERDR_WORKSPACE_ID`, `HERDR_TAB_ID`, `HERDR_PANE_ID`, and `HERDR_VIEW_ID`.
Action commands also receive `HERDR_PLUGIN_ACTION_ID`; startup and event hooks
receive `HERDR_PLUGIN_EVENT` (`startup` for startup hooks), event hooks also
...
agent, selected text, clicked URL, link handler, and client view fields when
they are available for that invocation. `HERDR_VIEW_ID` is an opaque,
connection-lifetime routing value for a full app client. It is not a user
identity or permission token, is absent from server-originated work, and changes
after reconnect. Shell plugins can read individual env vars for common fields
or parse the context JSON for the full shape.
...
**Execution providers.** A manifest can declare one or more
`[[execution_providers]]` entries. `scheme` uses the lowercase URL-scheme
grammar `[a-z][a-z0-9+.-]*`; `local` and `ssh` are reserved. `protocol` must be
a positive integer; current Herdr supports protocol `1`. Both `pty_command` and
`process_command` are required argv arrays. Herdr requires exactly one enabled,
platform-compatible provider for a scheme and reports missing, disabled,
incompatible, unsupported, or ambiguous providers before spawning the pane or
action. Protocol 1 execution-provider targets require a Unix Herdr server.

An extension target is written as `SCHEME:TARGET`, for example
`runtime:dev2`. The socket/API form is
`{"kind":"extension","scheme":"runtime","target":"dev2"}`. Herdr runs the
provider command locally from its plugin root with that provider plugin's
`HERDR_PLUGIN_ID`, `HERDR_PLUGIN_ROOT`, `HERDR_PLUGIN_CONFIG_DIR`,
`HERDR_PLUGIN_STATE_DIR`, and `HERDR_BIN_PATH` environment. It supplies one JSON
request of at most 16 KiB in `HERDR_EXECUTION_PROVIDER_REQUEST`:
...
```json
{
  ...
  "target": "dev2",
  "cwd": "/repo",
  "command": { "kind": "argv", "argv": ["omp"] },
  "env": [["HERDR_ENV", "1"]],
  "remove_env": ["BUN_OPTIONS"],
  "ready_nonce": "0123456789abcdef0123456789abcdef"
}
```

`command` is `shell`, `shell_command`, or `argv`. The PTY command must relay
terminal bytes and resize changes. After the target attaches, it must emit this
exact OSC frame using either BEL or ST as the terminator:

```text
ESC]6973;herdr-remote-exec-ready={"nonce":"<ready_nonce>","hostname":"<optional>","cwd":"<optional>"}BEL
ESC]6973;herdr-remote-exec-ready={"nonce":"<ready_nonce>","hostname":"<optional>","cwd":"<optional>"}ESC\
```

The JSON payload is limited to 32 KiB. `hostname` and `cwd` are optional; the
nonce must match the request. The process command is the non-TTY path used for
actions and must keep stdout and stderr separate and forward the exit status.
The provider owns target syntax, transport, authentication, and remote process
lifecycle. Herdr keeps rendering, layout, input, and persistence local.
...
closes. Manifest `scope` defaults to `shared`. A `plugin.pane.open` request can
override placement with `overlay`, `popup`, `workspace_right`, `split`, `tab`,
or `zoomed`, and can override scope with `shared` or `client_private`
(`--scope client-private` in the CLI).

With shared scope, `placement = "popup"` opens a session-modal terminal popup
without changing the tiled layout. It accepts optional `width` and `height`
fields in the manifest or open request; omit them for the default half-size
popup, use numbers for outer terminal-cell dimensions, or use strings like
`"80%"` for a percentage of the terminal area. It receives all terminal input,
including Escape, and closes when the command exits or a `popup.close` request
is sent. Dimensions smaller than the popup minimum are clamped.
...
`placement = "workspace_right"` opens one terminal pane on a workspace's right
edge, outside its tab split tree. Tab switches retain the pane; workspace
switches restore the pane and its focus state independently for each workspace.
Its content aligns with the workspace tab surface while its separator and
bottom-left collapse control span the workspace height. Drag the left separator
to resize it. The control collapses the pane to one column without stopping the
process and never changes focus. Opening with `focus = false` starts collapsed;
focusing the pane expands it. It accepts `width` but not `height`, using 30% when
omitted.
...
width = 32
command = ["herdr-edit", "--explorer"]
```

The process receives `HERDR_WORKSPACE_ID` and a stable `HERDR_PANE_ID` of the
form `<workspace_id>:plugin`, but no `HERDR_TAB_ID`. The pane is runtime-scoped:
it closes with its workspace, its process, or `plugin.pane.close`, and is not
restored from session snapshots. `plugin.pane.focus` switches to its workspace
and focuses it. A different plugin entrypoint cannot replace an occupied
workspace slot without closing the current pane first.

Pane commands receive the active palette as protected `HERDR_THEME_*`
environment variables. Values are `#rrggbb`, `reset`, a Ratatui named color such
as `blue` or `lightblue`, or `indexed:N`.

Split, tab, zoomed, and overlay plugin panes are normal shared Herdr panes after
they open. Plugins can call standard pane APIs such as `pane.move`, `pane.swap`,
...
workspaces. Workspace-right panes support plugin focus and close operations but
remain outside general pane, layout, persistence, and agent APIs.

A shared popup is a singleton session resource. It has no pane ID or pane
lifecycle events; its process does not receive `HERDR_PANE_ID`, and the
underlying tiled pane remains available through `HERDR_PLUGIN_CONTEXT_JSON`.
Opening a shared popup returns `ui_busy` while Settings, Copy mode, or another
Herdr modal is active. `popup.close` closes the shared popup, and
`plugin.pane.open` returns an `ok` result after launch.

For an independent popup owned by one connected app client, declare
client-private scope. This scope is valid only with popup placement:
...
command = ["herdr-browser"]
```

A client-private popup belongs to one connected full app client, and each client
may own one independent runtime. Its rendering, cursor, hyperlinks, input, and
terminal size are visible only to that client. A successful new private popup
replaces that client's old one; a failed launch leaves the old popup running.
The private popup closes when its command exits or its client disconnects, and
must be reopened after reconnect because the client gets a new view ID. It is
connection-scoped and is not snapshotted, restored, or handed off.

Both popup scopes have no public pane ID, do not change shared plugin focus
context, emit no shared pane lifecycle events, and do not participate in pane,
layout, persistence, or agent APIs. Their processes do not receive
`HERDR_PANE_ID`; the underlying tiled source pane remains available through
`HERDR_PLUGIN_CONTEXT_JSON`. A private popup exits through its own command,
disconnect, or successful replacement.
...
Use `[[link_handlers]]` to route a plain left click on a matching terminal URL
to a plugin action instead of opening the URL in the browser. `pattern` is a
Rust regular expression matched against the clicked URL, and `action` must
...
When a link click originates from a full app client, Herdr also preserves that
client's `view_id`. A handler can therefore open a client-private popup without
guessing which simultaneous client clicked the link. File and `artifact://`
URLs are passed to matching handlers unchanged; only safe HTTP(S) URLs use the
platform opener when no handler accepts them.
