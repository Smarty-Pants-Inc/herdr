...
`agent.prompt` writes to a pane where Pi reports through herdr's Pi integration and Pi
is the foreground process, herdr wraps the bytes in an origin frame, so Pi can tell
sent input from typed input:

```text
ESC _ herdr-origin;v=1;kind=api;id=<id>;sender=<caller>[;pane=<pane id>][;session=<session id>] ESC \
<input bytes>
...
Values are percent-encoded. Other programs receive the raw bytes. In all other
input, including keyboard input, pastes, and the payload inside a frame, herdr
replaces the last byte of any `ESC _ herdr-origin` prefix with `?`, so only herdr
can emit a frame.
