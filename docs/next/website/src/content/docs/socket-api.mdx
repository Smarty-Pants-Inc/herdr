...
only while its foreground job is the Pi job that reported `input_origin: "v1"`
through herdr's Pi integration (`pane.report_agent` from source `herdr:pi`). herdr
accepts that claim only from a process inside that job, checked through the socket
peer. Other programs, a later Pi process, and Pi versions that do not report it
