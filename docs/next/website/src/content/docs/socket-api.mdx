...
The markers are the Unicode noncharacters `U+FDD0` and `U+FDD1`. When herdr accepts a
Pi's claim, it sends that Pi `U+FDD0 herdr-origin;ready;v=1;pid=<pid> U+FDD1`, so the Pi knows
that its API input is framed from then on. A pane gets frames
