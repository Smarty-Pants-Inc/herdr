...
Values are percent-encoded. Other programs receive the raw bytes. herdr removes
the `ESC _ herdr-origin` prefix from all other input, including keyboard input,
pastes, and the payload inside a frame, so only herdr can emit a frame.
