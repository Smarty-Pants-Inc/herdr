...
only while its Pi reports `input_origin: "v1"` through herdr's Pi integration
(`pane.report_agent` from source `herdr:pi`) and Pi is the foreground process.
Other programs, and Pi versions that do not report it, receive the raw bytes.
