...
`agent.prompt` writes to a pane whose agent is Pi and Pi is the foreground process,
herdr wraps the bytes in an origin frame, so Pi can tell sent input from typed input:
