...
`pane.split` also accepts an optional `execution_target`:
`{"kind":"local"}`, on Unix `{"kind":"ssh","host":"HOST"}`, or
`{"kind":"extension","scheme":"SCHEME","target":"TARGET"}` for a
plugin-declared execution provider. When it is omitted, the new pane inherits
the source pane's target. An explicit `target_pane_id` supplies both the
insertion point and the source target/cwd. Managed workspace-right plugin
`caller_pane_id` locality applies only to a workspace-selected split without an
explicit target pane. A same-target split follows the source cwd; a cross-target
local split uses the configured `terminal.new_cwd`, while a cross-target remote
split uses the target provider's default cwd. Explicit `cwd` overrides either
default.

Per-pane SSH and extension targets require a Unix Herdr server. Extension
targets also require exactly one enabled, compatible installed plugin that
declares the requested scheme and supports the server platform. This does not
affect whole-server `herdr --remote`, which remains available on Windows clients
connecting to supported Linux and macOS hosts.
...
Every `PaneInfo` includes its `execution_target`, so clients can keep local,
SSH-backed, and extension-backed panes distinct without inferring locality from
paths or process metadata.
...
`pane_id`, `label`, `cwd`, argv `command`, and `execution_target`. The target is
`{"kind":"local"}` by default, `{"kind":"ssh","host":"HOST"}` for built-in
SSH, or `{"kind":"extension","scheme":"SCHEME","target":"TARGET"}` for an
installed execution provider. Split nodes use `direction` (`right` or `down`),
`ratio`, `first`, and `second`.
...
This restores structure, labels, cwd, execution targets, env, and optional argv
commands; it does not preserve live PTYs, scrollback, or running processes.
  ...
  "params": {
    ...
    "root": {
      ...
      "second": {
        ...
        "execution_target": { "kind": "ssh", "host": "build.example" },
...
`title` is required and must contain visible text after control characters and repeated whitespace are removed. `body` is optional. Herdr collapses newlines, tabs, carriage returns, and repeated whitespace into spaces, then trims notification text to 80 characters for `title` and 240 characters for `body`. An empty sanitized `title` returns `invalid_params`. `position` applies only when the effective toast delivery is `herdr`: configured `hybrid` delivery is in-app while the outer terminal is focused or its focus is unknown, and system delivery while it is unfocused. Desktop positions are relative to the full Herdr frame, and omitted positions use `ui.toast.herdr.position`. Terminal, system, and off delivery ignore `position`. `sound` is optional and can be `none`, `done`, or `request`; it defaults to `none` and plays only when the notification is shown.
...
Open a workspace-attached right pane:

```json
{"id":"req_plugin_right","method":"plugin.pane.open","params":{"plugin_id":"example.explorer","entrypoint":"explorer","placement":"workspace_right","workspace_id":"w1","width":"30%","focus":true}}
```

`plugin.pane.open` requires an installed, enabled plugin that is compatible
with the effective execution target. It launches the requested manifest
`[[panes]]` entrypoint as an argv-backed terminal surface. Manifest pane
`placement` defaults to `overlay`; request `placement` overrides it with
`overlay`, `popup`, `workspace_right`, `split`, `tab`, or `zoomed`. The optional
`scope` is `shared` by default. `client_private` is valid only for popup
placement and requires the connected app client's `view_id`. Overlay and popup
placements use the active tiled pane as launch context.

`workspace_right` targets the specified or active workspace and accepts `width`
but not `height`. Each workspace can own one right pane outside its tab split
tree. Tab switches retain it; workspace switches restore its independent focus
state. Opening returns `plugin_workspace_pane_opened` with a stable pane ID of
the form `<workspace_id>:plugin`; `plugin.pane.focus` and `plugin.pane.close`
accept that ID. Its process receives `HERDR_WORKSPACE_ID` and `HERDR_PANE_ID`
but not `HERDR_TAB_ID`. The pane closes with its workspace or process and is not
restored from session snapshots. It otherwise remains outside general
`pane.*`, layout, persistence, and agent APIs.

A shared popup is session-modal and does not change the tab layout. Optional
`width` and `height` fields set its outer size as terminal cells or percentages
such as `"80%"`. It has no pane ID, remains outside `pane.*` and agent APIs,
emits no pane lifecycle events, keeps plugin focus context on the underlying
tiled pane, and does not export `HERDR_PANE_ID`. Launch returns `ok`;
`popup.close` closes only this shared popup and returns `popup_not_open` when
none exists.

A `client_private` popup is owned by one connected full app client. Its
rendering, input, cursor, hyperlinks, and terminal size are visible only to
that client. Each client can own one; a successful replacement closes its old
popup, and disconnect closes it. Private popups are not snapshotted, restored,
or handed off, and they exit through their own process rather than
`popup.close`.

Split and zoomed panes target an existing pane; tab panes can target a
workspace. Split, tab, zoomed, and overlay panes behave like normal Herdr panes,
and `plugin.pane.focus` and `plugin.pane.close` continue to operate on them.
...
Session-only official integrations report agent session references with `pane.report_agent_session`. State-reporting integrations can still include native session references in `pane.report_agent`. State-independent session reports do not affect waits, notifications, or rollups.
...
`resume_policy` is optional and defaults to `native`, which preserves the existing
cold-restore behavior. Set it to `external` when another runtime owns session
lifecycle: Herdr persists and exposes the session reference but never creates a
native resume plan for it.

`pane.get`, `pane.list`, `agent.get`, and `agent.list` expose a read-only `agent_session` object when Herdr has a stored agent session reference:
...
If no session reference is stored, the field is omitted. Native session entries omit `resume_policy` to preserve the existing response shape; externally owned entries report `"resume_policy": "external"`.
