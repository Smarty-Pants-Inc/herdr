...
pane inherits the source pane's target. A same-target split follows the source
working directory; a cross-target split uses the destination's default home
unless `cwd` is explicit. Managed workspace-right plugin callers set
`caller_pane_id` so a workspace-selected split keeps the plugin's locality
while using the workspace's focused tiled pane as the insertion point.
