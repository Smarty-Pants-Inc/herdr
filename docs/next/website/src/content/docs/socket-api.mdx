...
Before `pane.send_text`, `pane.send_input`, `pane.send_keys`, `agent.send_keys`, or
`agent.prompt` writes to a pane, herdr appends one JSON line to `api-input.jsonl` in its
state directory (for example `~/.local/state/herdr/api-input.jsonl`; the file is readable
by its owner only). The line records the time (`ts_ms`), the method, the target pane and
terminal, the byte count, and the caller as herdr knows it from the socket peer process:
its pid, pane, agent name, and agent session. It never records the text. If herdr cannot
write the line, it refuses the input with `input_log_unavailable`.

A reader of an agent's session can then tell turns that the API sent into a pane from
turns typed there, without anything in the terminal byte stream.
