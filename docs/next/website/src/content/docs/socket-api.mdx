...
`pane.split` also accepts an optional `execution_target`: `{"kind":"local"}`
or, on Unix, `{"kind":"ssh","host":"HOST"}`. When it is omitted, the new
pane inherits the source pane's target. An explicit `target_pane_id` supplies
both the insertion point and the source target/cwd. Managed workspace-right
plugin `caller_pane_id` locality applies only to a workspace-selected split
without an explicit target pane. A same-target split follows the source cwd; a
cross-target local split uses the configured `terminal.new_cwd`, while a
cross-target SSH split starts in the remote user's home. Explicit `cwd`
overrides either default.

Per-pane SSH execution targets are Unix-only. This does not affect whole-server
...
Every `PaneInfo` includes its `execution_target`, so clients can keep local and
SSH-backed panes distinct without inferring locality from paths or process
metadata.
...
`{"kind":"local"}` by default or, on Unix, `{"kind":"ssh","host":"HOST"}`.
Split nodes use `direction` (`right` or `down`), `ratio`, `first`, and `second`.
