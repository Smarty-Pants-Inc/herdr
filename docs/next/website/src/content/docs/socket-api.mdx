...
| Media | `pane.media_open`, `media.answer`, `media.mute`, `media.state`, `media.close` |

`pane.media_open` asks the attached Herdr client whose input last reached the pane, within 10 seconds, to act as the WebRTC audio peer for that pane: it captures the microphone and plays the reply on the user's machine, and the server carries only SDP and control. It routes only to that client, and only when it advertises native media support (`media.webrtc.v1`). The response is `media_offer` with a `session_id` and the client's SDP offer. Post the remote answer with `media.answer`, mute with `media.mute`, poll `media.state` (`opening`, `offered`, `connecting`, `connected`, `failed` or `closed` with a `code`), and end the call with `media.close`, which releases the microphone. Errors: `media_no_client` (no recent input from an attached client), `media_unsupported_client` (the client that typed last has no media support), `media_refused` (the client declined; the message starts with a code such as `declined`, `disabled`, `not_viewed` or `stale_input`) and `media_timeout`. The client setting `media = "off" | "ask" | "auto"` controls consent; `ask`, the default, prompts once per client run.
...
When `pane.send_text`, `pane.send_input`, `pane.send_keys`, `agent.send_keys`, or
`agent.prompt` writes to a pane whose Pi reads origin frames, herdr wraps the bytes
in an origin frame, so Pi can tell sent input from typed input:

```text
U+FDD0 herdr-origin;v=1;kind=api;id=<id>;sender=<caller>[;pane=<pane id>][;session=<session id>] U+FDD1
<input bytes>
U+FDD0 herdr-origin;end;id=<id> U+FDD1
```

The markers are the Unicode noncharacters `U+FDD0` and `U+FDD1`. The Pi's report also
carries `input_origin_nonce`, a random value for that Pi run. When herdr accepts the claim, it
sends `U+FDD0 herdr-origin;ready;v=1;nonce=<nonce> U+FDD1`, so that Pi, and no other reader,
knows that its API input is framed from then on. A pane gets frames
only while the Pi process that reported `input_origin: "v1"` through herdr's Pi
integration (`pane.report_agent` from source `herdr:pi`) is a Pi in the pane's
foreground job. herdr accepts that claim only when the socket peer is that Pi
process itself, and no other process can replace a claim while its Pi still runs
there. Other programs, a later Pi process, and Pi versions that do not report it
receive the raw bytes. If herdr cannot read the foreground job of a pane with a
claim, it refuses the write with `input_origin_unavailable` instead of sending
raw bytes.

`sender` is the calling pane's agent name, else its pane id, else `pid:<n>`, else
`unknown`. herdr takes it from the socket peer process, never from request text.
Values are percent-encoded. In all other input, including keyboard input, pastes,
and the payload inside a frame, herdr replaces the last byte of any `U+FDD0` with
`?`, so only herdr can start a frame.
