...
herdr pane split [<pane_id>|--pane ID|--current] --direction right|down [--workspace WORKSPACE] [--ratio FLOAT] [--cwd PATH] [--target local|ssh:HOST] [--env KEY=VALUE] [--focus] [--no-focus]
...
`--current` を受け付けるペインコマンドは、Herdr ペイン内から実行した場合、呼び出し元の `HERDR_PANE_ID` を使います。`pane split` では、明示したペイン ID、`--pane ID`、または `--current` で選択したペインが、挿入位置と継承する実行ターゲット/cwd の両方を決めます。ペイン選択を省略すると UI でフォーカス中のペインを使います。分割レスポンスでは、新しいペイン ID は `.result.pane.pane_id` にあります。

`--workspace` は、そのワークスペースでフォーカス中のタイルペインを挿入位置として選びます。管理対象のワークスペース右側プラグインが、明示的なペイン選択なしでワークスペース指定の分割を呼び出した場合、新しいペインはワークスペースでフォーカス中のタイルペインを分割しながら、プラグイン呼び出し元の実行ターゲットと cwd を継承できます。

ペイン単位の SSH 実行ターゲットは Unix 専用です。これはサーバー全体を対象とする `herdr --remote` とは別の機能で、後者は Windows クライアントから対応する Linux/macOS ホストへの接続でも利用できます。

`--target` を省略すると、分割はソースペインの実行ターゲットを継承します。`--target local` でローカル実行を強制し、`--target ssh:HOST` で現在の Herdr UI 内に置いたまま信頼済み SSH ホスト上で新しいターミナルを実行できます。同じターゲットへの分割はソース cwd を引き継ぎます。ターゲットをまたぐローカル分割は設定済みの `terminal.new_cwd` を使い、ターゲットをまたぐ SSH 分割はリモートユーザーのホームから始まります。明示的な `--cwd` はどちらのデフォルトも上書きします。先に `herdr --remote HOST` を実行して対応するリモート Herdr を準備してください。ペイン単位の起動はリモートソフトウェアをインストールも置換もしません。非対話 OpenSSH と接続/リクエスト配信の制限時間を使うため、鍵または `ssh-agent` で事前に認証できる必要があります。詳細は[永続化とリモートアクセス](/ja/docs/persistence-remote/)を参照してください。

`pane get` と `pane list` は各ペインの `execution_target` を報告します。リモート cwd はリモートターミナルから報告され、サーバーのファイルシステムでは検証されません。`pane process-info` はローカル専用で、リモートペインはローカルのプロセス情報ではなく `pane_process_info_unsupported` を返します。
...
`agent start` は既存の利用可能なシェルペインを起動対象にします。対話シェル自身がフォアグラウンドを所有し、フォアグラウンドのコマンド、エディタ、エージェントが動いていない必要があります。ローカル、SSH バックエンド、実行プロバイダーバックエンドのペインをサポートします。リモートペインでは POSIX 互換の対話シェルが必要で、Herdr はシェルセーフな起動コマンドを送り、ローカルのプロセステーブルではなくターミナルのライフサイクル検出を使います。トポロジーは別に作成します。名前はライブエージェント間で一意で、`[a-z][a-z0-9_-]{0,31}` に一致する必要があります。対応する kind は `pi`、`claude`、`codex`、`gemini`、`cursor`、`devin`、`agy`、`cline`、`omp`、`mastracode`、`opencode`、`copilot`、`kimi`、`kiro`、`droid`、`amp`、`grok`、`hermes`、`kilo`、`qodercli`、`qwen`、`maki` です。名前は現在のペイン占有者に属し、そのエージェントの終了、release、置換で消えます。一時的に検出できないだけでは消えません。
