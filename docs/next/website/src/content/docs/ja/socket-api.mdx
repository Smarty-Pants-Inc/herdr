...
マーカーは Unicode の非文字 `U+FDD0` と `U+FDD1` です。ペインにフレームが送られるのは、その Pi が herdr の Pi インテグレーション（ソース `herdr:pi` の `pane.report_agent`）で `input_origin: "v1"` を報告し、かつ Pi がフォアグラウンドプロセスである間だけです。ほかのプログラムや、これを報告しない Pi には生のバイト列が届きます。
