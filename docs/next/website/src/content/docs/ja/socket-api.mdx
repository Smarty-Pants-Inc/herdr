...
マーカーは Unicode の非文字 `U+FDD0` と `U+FDD1` です。ペインにフレームが送られるのは、herdr の Pi インテグレーション（ソース `herdr:pi` の `pane.report_agent`）で `input_origin: "v1"` を報告した Pi プロセスが、そのペインのフォアグラウンドジョブ内の Pi である間だけです。herdr はこの報告を、ソケットのピアがその Pi プロセス自身である場合にのみ受け付けます。ほかのプログラムや、これを報告しない Pi には生のバイト列が届きます。
