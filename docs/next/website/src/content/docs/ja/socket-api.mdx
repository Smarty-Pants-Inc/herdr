...
`pane.send_text`、`pane.send_input`、`pane.send_keys`、`agent.send_keys`、`agent.prompt` が、Pi が herdr の Pi インテグレーション経由で状態を報告し、かつ Pi がフォアグラウンドプロセスであるペインに書き込むとき、herdr はバイト列を入力元フレームで囲みます。これにより Pi は送信された入力と打鍵された入力を区別できます。
...
`sender` は呼び出し元ペインのエージェント名、なければペイン ID、なければ `pid:<n>`、なければ `unknown` です。herdr はこれをリクエスト本文ではなくソケットのピアプロセスから決めます。値はパーセントエンコードされます。ほかのプログラムには生のバイト列が届きます。herdr はキーボード入力、ペースト、フレーム内のペイロードを含むほかのすべての入力に含まれる `ESC _ herdr-origin` 接頭辞の最後のバイトを `?` に置き換えるため、フレームを出せるのは herdr だけです。
