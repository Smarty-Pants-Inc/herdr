...
`sender` は呼び出し元ペインのエージェント名、なければペイン ID、なければ `pid:<n>`、なければ `unknown` です。herdr はこれをリクエスト本文ではなくソケットのピアプロセスから決めます。値はパーセントエンコードされます。ほかのプログラムには生のバイト列が届きます。herdr はキーボード入力、ペースト、フレーム内のペイロードを含むほかのすべての入力から `ESC _ herdr-origin` 接頭辞を取り除くため、フレームを出せるのは herdr だけです。
