...
An available shell pane is at its interactive shell prompt: the shell itself owns the foreground, with no foreground command, editor, or agent running. Return the pane to its prompt before calling `agent start`. Local, SSH-backed, and execution-provider-backed panes are supported. Remote panes must use a POSIX-compatible interactive shell; Herdr relies on terminal lifecycle detection instead of local process-table inspection.
