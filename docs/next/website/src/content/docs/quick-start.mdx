...
Resolved OSC 8 and visible `http://`/`https://` links underline on hover and open with one click; unhandled HTTP(S) opens on the clicking client, while plugin-handled `file://` links stay in the workspace.
