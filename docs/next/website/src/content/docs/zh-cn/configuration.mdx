...
默认键位以前缀为主，因此 Herdr 不会抢占 shell、编辑器、tmux 或终端应用的输入。Findr 是有意保留的终端模式直接绑定例外：macOS 默认 `cmd+f`，其他平台默认 `ctrl+alt+f`。在[配置参考](/docs/config-reference/)中搜索 `keys.` 可查看每个动作及其默认绑定。应用内帮助面板可通过 `prefix+?` 打开，其中会显示当前生效的绑定。
