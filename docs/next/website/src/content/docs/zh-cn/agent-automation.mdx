...
可用的 shell 窗格必须停在交互式 shell 提示符,由 shell 自身占用前台,没有正在前台运行的命令、编辑器或智能体。调用 `agent start` 前先让窗格回到提示符。本地窗格和执行提供程序后端窗格受支持。内置 SSH 窗格会返回 `agent_start_unsupported_execution_target`。
