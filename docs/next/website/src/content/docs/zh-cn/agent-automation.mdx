...
可用的 shell 窗格必须停在交互式 shell 提示符,由 shell 自身占用前台,没有正在前台运行的命令、编辑器或智能体。调用 `agent start` 前先让窗格回到提示符。`agent start` 目前只支持本地窗格;SSH 后端目标会返回 `agent_start_unsupported_execution_target`。
