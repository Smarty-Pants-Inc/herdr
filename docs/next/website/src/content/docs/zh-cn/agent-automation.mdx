...
可用的 shell 窗格必须停在交互式 shell 提示符,由 shell 自身占用前台,没有正在前台运行的命令、编辑器或智能体。调用 `agent start` 前先让窗格回到提示符。本地、SSH 后端和执行提供程序后端窗格都受支持。远程窗格必须使用兼容 POSIX 的交互式 shell;Herdr 依靠终端生命周期检测,而不是本地进程表。
