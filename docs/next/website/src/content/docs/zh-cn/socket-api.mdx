...
响应包含 `workspace_id`、`tab_id`、`zoomed`、`focused_pane_id` 和 `root`。`root` 是由 `pane` 和 `split` 节点组成的 BSP 树。窗格节点可以包含 `pane_id`、`label`、`cwd`、argv `command` 和 `execution_target`。目标默认为 `{"kind":"local"}`;在 Unix 上也可以使用 `{"kind":"ssh","host":"HOST"}`。分割节点使用 `direction` (`right` 或 `down`)、`ratio`、`first` 和 `second`。

`layout.apply` 从声明式的树创建一个新标签页。提供 `tab_id` 时,Herdr 先创建替代标签页,再关闭旧标签页。它会恢复结构、标签、cwd、执行目标、env 和可选的 argv 命令;不会保留活跃的 PTY、回滚内容或运行中的进程。
