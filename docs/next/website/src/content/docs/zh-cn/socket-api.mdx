...
| 媒体 | `pane.media_open`、`media.answer`、`media.mute`、`media.state`、`media.close` |
...
当 `pane.send_text`、`pane.send_input`、`pane.send_keys`、`agent.send_keys` 或 `agent.prompt` 写入一个其 Pi 能读取输入来源帧的窗格时，herdr 会用输入来源帧包裹这些字节，让 Pi 能区分发送的输入和键入的输入：
...
标记是 Unicode 非字符 `U+FDD0` 和 `U+FDD1`。Pi 的上报还带有本次运行的随机值 `input_origin_nonce`。herdr 接受声明后，会发送 `U+FDD0 herdr-origin;ready;v=1;nonce=<nonce> U+FDD1`，只告知该 Pi 此后的 API 输入都会带帧。只有当通过 herdr 的 Pi 集成（来源为 `herdr:pi` 的 `pane.report_agent`）上报了 `input_origin: "v1"` 的那个 Pi 进程仍是窗格前台作业中的 Pi 时，该窗格才会收到帧。herdr 只在 socket 对端就是该 Pi 进程本身时才接受这一上报，并且在该 Pi 仍在运行时，其他进程不能替换它的声明。如果 herdr 无法读取已声明窗格的前台作业，它会以 `input_origin_unavailable` 拒绝写入，而不是发送原始字节。其他程序以及不上报它的 Pi 收到的是原始字节。

`sender` 是调用方窗格的智能体名称；没有时为窗格 ID，再没有时为 `pid:<n>`，否则为 `unknown`。herdr 从 socket 对端进程得出它，从不取自请求文本。值经过百分号编码。herdr 会把所有其他输入（包括键盘输入、粘贴和帧内的负载）中任何 `U+FDD0` 的最后一个字节替换为 `?`，因此只有 herdr 能开始一个帧。
