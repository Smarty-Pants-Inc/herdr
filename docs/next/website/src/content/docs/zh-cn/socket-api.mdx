...
标记是 Unicode 非字符 `U+FDD0` 和 `U+FDD1`。只有当通过 herdr 的 Pi 集成（来源为 `herdr:pi` 的 `pane.report_agent`）上报了 `input_origin: "v1"` 的那个 Pi 进程仍是窗格前台作业中的 Pi 时，该窗格才会收到帧。herdr 只在 socket 对端就是该 Pi 进程本身时才接受这一上报。其他程序以及不上报它的 Pi 收到的是原始字节。
