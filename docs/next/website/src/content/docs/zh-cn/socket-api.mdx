...
标记是 Unicode 非字符 `U+FDD0` 和 `U+FDD1`。只有当窗格中的 Pi 通过 herdr 的 Pi 集成（来源为 `herdr:pi` 的 `pane.report_agent`）上报 `input_origin: "v1"`，且 Pi 是前台进程时，该窗格才会收到帧。其他程序以及不上报它的 Pi 收到的是原始字节。
