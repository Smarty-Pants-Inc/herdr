...
标记是 Unicode 非字符 `U+FDD0` 和 `U+FDD1`。只有当窗格的前台作业是通过 herdr 的 Pi 集成（来源为 `herdr:pi` 的 `pane.report_agent`）上报了 `input_origin: "v1"` 的那个 Pi 作业时，该窗格才会收到帧。herdr 只接受由该作业内的进程发出的上报，并通过 socket 对端核实。其他程序以及不上报它的 Pi 收到的是原始字节。
