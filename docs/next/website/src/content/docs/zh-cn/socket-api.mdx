...
当 `pane.send_text`、`pane.send_input`、`pane.send_keys`、`agent.send_keys` 或 `agent.prompt` 写入一个窗格，且该窗格中的 Pi 通过 herdr 的 Pi 集成上报状态、Pi 是前台进程时，herdr 会用输入来源帧包裹这些字节，让 Pi 能区分发送的输入和键入的输入：
...
`sender` 是调用方窗格的智能体名称；没有时为窗格 ID，再没有时为 `pid:<n>`，否则为 `unknown`。herdr 从 socket 对端进程得出它，从不取自请求文本。值经过百分号编码。其他程序收到的是原始字节。herdr 会从所有其他输入（包括键盘输入、粘贴和帧内的负载）中把 `ESC _ herdr-origin` 前缀的最后一个字节替换为 `?`，因此只有 herdr 能发出帧。
