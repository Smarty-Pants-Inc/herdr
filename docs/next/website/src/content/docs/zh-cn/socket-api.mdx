...
标记是 Unicode 非字符 `U+FDD0` 和 `U+FDD1`。herdr 接受某个 Pi 的声明后，会向该 Pi 发送 `U+FDD0 herdr-origin;ready;v=1;pid=<pid> U+FDD1`，告知其此后的 API 输入都会带帧。只有当通过 herdr 的 Pi 集成（来源为 `herdr:pi` 的 `pane.report_agent`）上报了 `input_origin: "v1"` 的那个 Pi 进程仍是窗格前台作业中的 Pi 时，该窗格才会收到帧。herdr 只在 socket 对端就是该 Pi 进程本身时才接受这一上报，并且在该 Pi 仍在运行时，其他进程不能替换它的声明。如果 herdr 无法读取已声明窗格的前台作业，它会以 `input_origin_unavailable` 拒绝写入，而不是发送原始字节。其他程序以及不上报它的 Pi 收到的是原始字节。
