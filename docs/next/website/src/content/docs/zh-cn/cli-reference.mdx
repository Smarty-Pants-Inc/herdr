...
接受 `--current` 的窗格命令在 Herdr 窗格内运行时使用调用方的 `HERDR_PANE_ID`。对 `pane split` 而言,显式窗格 ID、`--pane ID` 或 `--current` 选中的窗格同时决定插入位置以及继承的执行目标/cwd;省略窗格选择器则使用 UI 当前聚焦的窗格。拆分响应中的新窗格 ID 位于 `.result.pane.pane_id`。

`--workspace` 使用该工作区当前聚焦的平铺窗格作为插入位置。受管理的工作区右侧插件在没有显式窗格选择器时发起工作区指定的拆分,新窗格可以继承插件调用方的执行目标和 cwd,同时仍拆分该工作区聚焦的平铺窗格。

逐窗格 SSH 执行目标仅支持 Unix。这与整台服务器的 `herdr --remote` 不同;后者仍支持 Windows 客户端连接到受支持的 Linux 和 macOS 主机。

省略 `--target` 时,拆分会继承来源窗格的执行目标。使用 `--target local` 强制本地执行,或使用 `--target ssh:HOST` 在受信任的 SSH 主机上运行新终端,同时仍保留在当前 Herdr UI 中。同目标拆分沿用来源 cwd。跨目标的本地拆分使用配置的 `terminal.new_cwd`;跨目标的 SSH 拆分从远程用户的主目录开始。显式 `--cwd` 会覆盖两种默认值。请先运行 `herdr --remote HOST` 准备匹配的远程 Herdr;逐窗格启动不会安装或替换远程软件。它使用非交互 OpenSSH,并限制连接和请求投递时间,因此必须预先通过密钥或 `ssh-agent` 提供身份验证。详见[持久化与远程访问](/zh-cn/docs/persistence-remote/)。

`pane get` 和 `pane list` 会报告每个窗格的 `execution_target`。远程 cwd 来自远程终端的报告,不会对照服务器文件系统检查。`pane process-info` 仅支持本地窗格;远程窗格返回 `pane_process_info_unsupported`,而不是本地进程信息。
