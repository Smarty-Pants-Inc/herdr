...
mod tests {
    ...
    use crate::api::schema::{
        AgentPromptParams, AgentSendKeysParams, AgentStartParams, ErrorResponse, PaneLayoutParams,
    ...
    #[tokio::test]
    async fn unguarded_requests_skip_peer_pid_session_attribution() {
        let fixture = attributed_agent_fixture();
        ...
        let requests = [
            ...
            Request {
                id: "non-content-write".into(),
                method: Method::PaneLayout(PaneLayoutParams::default()),
            },
        ];

        for request in requests {
            crate::app::terminal_targets::test_reset_peer_pid_session_process_checks();
            assert!(fixture
                .app
                .cross_pane_input_denial(&request, context)
                .is_none());
            assert_eq!(
                crate::app::terminal_targets::test_peer_pid_session_process_checks(),
                0,
                "{} inspected peer session processes",
                request.id
            );
        }
    }
