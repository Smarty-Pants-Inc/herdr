...
impl Fixture {
    fn new() -> Self {
        ...
        run_git(&repo, &["worktree", "add", "--quiet", "--detach", checkout.to_str().unwrap()]);
        ...
        let params = WorktreeAdoptParams {
            ...
            repo_key: app.state.workspaces[0].worktree_space.as_ref().unwrap().key.clone(),
            target: observations[1][0].binding.clone(),
            known_workspace_id: app.public_workspace_id(2),
            known_panes: observations[2].iter().map(|pane| pane.binding.clone()).collect(),
        };
        Self { app, repo, params, observations }
    }

    fn adopt(&mut self) -> serde_json::Value {
        let observations = &self.observations;
        serde_json::from_str(&self.app.handle_worktree_adopt_with(
            "test".into(), self.params.clone(), |_, index| Ok(observations[index].clone()),
        )).unwrap()
    }
}
...
fn explicit_adoption_preserves_four_panes_and_replay_keeps_path_lookup_ambiguous() {
    ...
    for _ in 0..2 {
        ...
        assert_eq!(reply["result"]["workspace"]["workspace_id"], fixture.params.workspace_id);
        assert_eq!(reply["result"]["root_pane"]["terminal_id"], fixture.params.target.terminal_id);
        ...
        let result = fixture.app.lookup_worktree_checkout_with(
            &source, Path::new(&fixture.params.path), |_, _, _| Ok(None),
        );
...
fn explicit_adoption_rejects_wrong_expected_identity_and_extra_known_panes() {
    for change in 0..12 {
        ...
        match change {
            ...
            3 => { fixture.params.known_panes.pop(); },
            4 => fixture.params.known_panes.push(fixture.params.known_panes[0].clone()),
            ...
            _ => fixture.app.state.workspaces[2].worktree_space.as_mut().unwrap().key.push('x'),
        }
        assert_eq!(fixture.adopt()["error"]["code"], "worktree_adoption_unavailable");
    ...
    for change in 0..5 {
        ...
        match change {
            0 | 1 => {
                ...
                fixture.app.state.terminals.get_mut(&terminal).unwrap().agent_name = Some("pi".into());
            }
            2 => {
            ...
            _ => { fixture.app.state.workspaces[1].test_add_tab(None); }
        }
...
fn explicit_methods_do_not_search_another_server_for_a_missing_target() {
    ...
    let response = other_server.app.handle_worktree_adopt_with(
        "wrong-server".into(), target_server.params.clone(), |_, _| {
    ...
    let params = WorktreeVerifyAdoptionParams {
        binding: target_server.params.clone(), agent_session_id: None,
    };
    let response = other_server.app.verify_adoption_with("wrong-server".into(), &params, |_, _| {
        panic!("missing target must refuse before native observation")
    });
    let reply: serde_json::Value = serde_json::from_str(&response).unwrap();
    assert_eq!(reply["error"]["code"], "pane_not_found");
    assert!(other_server.app.state.workspaces[1].worktree_space.is_none());
...
fn explicit_verification_is_read_only_and_requires_existing_membership() {
    ...
    let response = fixture.app.verify_adoption_with("test".into(), &params, |_, index| {
    ...
    assert_eq!(reply["result"]["worktrees"][0]["open_workspace_id"], params.binding.workspace_id);
    assert_eq!(reply["result"]["worktrees"][1]["open_workspace_id"], params.binding.known_workspace_id);
...
fn explicit_verification_allows_only_the_expected_live_pi_and_its_tool_descendants() {
    ...
    let terminal = tab.terminal_id(tab.root_pane).unwrap().clone();
    fixture.app.state.terminals.get_mut(&terminal).unwrap().persisted_agent_session =
    ...
    let worker_job = ForegroundJob {
        ...
        processes: vec![ForegroundProcess {
            pid: 150, name: "pi".into(), argv0: Some("pi".into()),
            argv: Some(vec!["pi".into()]), cmdline: Some("pi".into()),
        }],
    };
    ...
    pane.worker = Some(LiveWorker {
        pid: 150, session_id: "retained-pi".into(),
        processes: vec![(150, worker_identity, worker_job), (151, tool_identity, tool_job)],
    });
    let params = WorktreeVerifyAdoptionParams {
        binding: fixture.params.clone(), agent_session_id: Some("retained-pi".into()),
    };
    for change in 0..5 {
        ...
        match change {
            ...
            2 => observations[1][0].worker.as_mut().unwrap().processes[1].1.parent_pid = 100,
            3 => observations[1][0].worker.as_mut().unwrap().processes[1].1.parent_pid = 151,
            _ => observations[1][0].worker.as_mut().unwrap().processes[1].2.processes[0].name = "pi".into(),
        }
        fixture.app.state.session_dirty = false;
        let events = fixture.app.event_hub.events_after(0).len();
        let response = fixture.app.verify_adoption_with("test".into(), &params, |_, index| {
...
fn explicit_adoption_rejects_native_process_cwd_and_second_observation_changes() {
    for change in 0..7 {
        ...
        let response = fixture.app.handle_worktree_adopt_with(
            "test".into(), fixture.params.clone(), |_, index| {
                calls += 1;
