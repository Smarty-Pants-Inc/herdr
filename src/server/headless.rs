...
impl HeadlessServer {
    ...
    /// Drains internal events and forwards client notifications.
    ...
    fn handle_client_input_events(
        ...
        if events.is_empty() {
            return native_consumed || private_consumed || identity_changed;
        }
...
mod tests {
    ...
    #[test]
    fn native_bound_app_masks_host_pane_and_consumes_terminal_input() {
        let mut server = test_headless_server();
    ...
    #[test]
    fn native_renderer_detach_immediately_restores_private_guest_without_duplicate_on_connection_detach(
    ) {
        let mut server = test_headless_server();
    ...
    #[test]
    fn host_stop_reconciles_private_guest_to_started_replacement() {
        let mut server = test_headless_server();
        ...
        let host_started = |host_id, session: &str| {
            let listener = TcpListener::bind("127.0.0.1:0").expect("bind OMP host listener");
            let _peer = std::net::TcpStream::connect(listener.local_addr().unwrap())
                .expect("connect OMP host peer");
            let (socket, _) = listener.accept().expect("accept OMP host peer");
