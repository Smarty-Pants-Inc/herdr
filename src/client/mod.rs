...
pub(crate) fn requested_render_encoding() -> RenderEncoding {
...
#[cfg(unix)]
pub(crate) fn client_omp_renderer_eligible(
...
fn retain_detached_process_after_wait(
    ...
    match result {
        ...
        Err(err) => {
            warn!(pid, err = %err, "failed to reap detached process");
            false
        }
    }
}
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn echild_detached_process_wait_drops_child() {
        let echild = io::Error::from_raw_os_error(libc::ECHILD);

        assert!(!retain_detached_process_after_wait(42, Err(echild)));
    }
