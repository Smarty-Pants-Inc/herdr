    ...
    while !should_quit.load(Ordering::Acquire) {
        ...
        match host_input_state.read_input(&mut scratch) {
            ...
            Ok((n, input_state)) => {
                ...
                if poll_read_ready(stdin_fd, timeout_ms) == Some(false) {
                    let input_state =
                        pending_input_state.unwrap_or_else(|| host_input_state.load());
...
fn send_framed_input(
    ...
    if let Some(pending_state) = *pending_input_state {
        ...
        if continued_state != input_state {
            let mut old_pending_bytes = framer.pending_len();
            while offset < bytes.len() && old_pending_bytes > 0 {
                let before_len = framer.pending_len();
                let chunks = framer.push(&bytes[offset..offset + 1]);
                offset += 1;
                let consumed = before_len
                    .saturating_add(1)
...
mod tests {
    ...
    fn stdin_input_event_carries_raw_bytes_and_snapshot() {
        ...
        let event = ClientLoopEvent::StdinInput(data.clone(), input_state);
        match event {
            ClientLoopEvent::StdinInput(d, captured) => {
                assert_eq!(d, data);
                assert_eq!(captured, input_state);
            }
    ...
    fn pixel_mouse_classification_is_narrow_and_uses_read_geometry() {
        ...
        let input_state = HostInputSnapshot::from_parts(7, true, true);
        let report = b"\x1b[<35;321;241M".to_vec();
        let Some(ClientLoopEvent::PixelMouse(data, captured, captured_state)) =
            classify_unix_input(report.clone(), input_state, Some(geometry))
        else {
        ...
        assert!(classify_unix_input(report, input_state, None).is_none());
        ...
        for raw in [
            ...
            let Some(ClientLoopEvent::StdinInput(data, captured_state)) =
                classify_unix_input(raw.to_vec(), input_state, Some(geometry))
            else {
    ...
    fn mouse_mode_transition_keeps_input_emitted_while_mode_is_applied() {
        ...
        let current = input_state
            .transition_mouse_mode(true, false, || writer.write_all(report))
            .unwrap();
        let (data, snapshot) = read_exact_input(&input_state, report.len());
    ...
    fn only_raw_framer_pending_input_retains_a_snapshot() {
        ...
        assert_eq!(
            framer.has_pending_input().then_some(input_state),
            Some(input_state)
        );

        let mut matcher = super::super::direct_graphics::ResponseMatcher::default();
        assert!(matcher.arm(19, 49));
        let mut direct_filter = super::super::direct_graphics::InputFilter::default();
        let _ = direct_filter.push(b"\x1b_Gi=49;", &mut matcher);
        assert!(direct_filter.has_pending());
        let empty_framer = crate::raw_input::RawInputByteFramer::default();
        assert_eq!(
            empty_framer.has_pending_input().then_some(input_state),
            None
        );
    }

    #[test]
    fn fresh_framer_tail_after_generation_change_uses_current_snapshot() {
        let host_input_state = HostInputState::new(true, true);
        let old = host_input_state.load();
        let geometry = crate::input::mouse::HostGeometry::new(80, 24, 800, 480).unwrap();
        let mut framer = crate::raw_input::RawInputByteFramer::default();
        let mut pending_input_state = None;
        let mut last_geometry = None;
        ...
        assert_eq!(report, b"\x1b[<0;2;2M");
        assert_eq!(captured_geometry, geometry);
        assert_eq!(mouse_state, current);
        assert_eq!(pending_input_state, None);
    }

    #[test]
    fn fresh_escape_retained_after_old_escape_uses_current_snapshot() {
        let host_input_state = HostInputState::new(true, true);
        let old = host_input_state.load();
        let geometry = crate::input::mouse::HostGeometry::new(80, 24, 800, 480).unwrap();
        let mut framer = crate::raw_input::RawInputByteFramer::test_with_split_coalesced_escape();
        let mut pending_input_state = None;
        let mut last_geometry = None;
        let mut pending_palette = Vec::new();
        let (tx, mut rx) = mpsc::channel(4);
